...
(define (install-rational-package)
    ...
    (define (make-rat n d)
        (let ((reduced (reduce n d)))
            ; i dont like the following car and cadr operations, because we dont know at this layer
            ; that reduced generates a list, do we?
...
(define (install-dense-term-list-package)
    ...
    (define (pseudoremainder-terms-dense a b)
        (let ((term-a (first-term-dense a)) (term-b (first-term-dense b)))
            (let ((int-factor (expt (coeff term-b) (- (1+ (order term-a)) (order term-b)))))
    ...
    ; I dont like the following function because it is similar to the previous
    ; one, but because of time I did not reutilize logic
    (define (gcd-apply-terms-dense-2 a b)
        (let ((gcd-integer (apply gcd (append a b))))
            (map (lambda (terms) (map (lambda (term) (/ term gcd-integer)) terms)) (list a b))
        )
    )

    (define (reduce-terms-dense a b)
        (let ((gcd-a-b (gcd-terms-dense a b)))
            (let ((first-gcd-term (first-term-dense gcd-a-b))
                    (first-a (first-term-dense a))
                    (first-b (first-term-dense b)))
                (let ((int-factor (expt (coeff first-gcd-term) (- (1+ (max (order first-a) (order first-b))) (order first-gcd-term)))))
                    ; I dont like the following car operations because they violate the abstraction barrier, there should be a result interface (as the remainder interface)
                    (let ((new-a (car (div-terms-dense (mul-term-by-all-terms-dense (make-term 0 int-factor) a) gcd-a-b)))
                            (new-b (car (div-terms-dense (mul-term-by-all-terms-dense (make-term 0 int-factor) b) gcd-a-b))))
                        (gcd-apply-terms-dense-2 new-a new-b)
                    )
                )
            )
        )
    ...
    (put 'reduce-terms '(dense dense) (lambda (term-list1 term-list2) (map tag (reduce-terms-dense term-list1 term-list2))))
)
...
(define (install-polynomial-package)
    ...
    (define (reduce-poly p1 p2)
        (if (same-variable? (variable p1) (variable p2))
            (map (lambda (terms) (make-poly (variable p1) terms)) (reduce-terms (term-list p1) (term-list p2)))
...
(define (reduce-terms term-list1 term-list2) (apply-generic 'reduce-terms term-list1 term-list2))
(define (reduce term-list1 term-list2) (apply-generic 'reduce term-list1 term-list2))


(define p1 (make-polynomial-dense 'x '(1 1)))
(define p2 (make-polynomial-dense 'x '(1 0 0 -1)))
(define p3 (make-polynomial-dense 'x '(1 0)))
(define p4 (make-polynomial-dense 'x '(1 0 -1)))
...
(define p11 (make-polynomial-dense 'x '(1 0)))
(define p22 (make-polynomial-dense 'x '(1 -1)))
(define p33 (make-polynomial-dense 'x '(-1)))
(define p44 (make-polynomial-dense 'x '(1 -1)))

(define rf11 (make-rational p11 p22))
(define rf22 (make-rational p33 p44))

(define rf-sum-2 (add rf11 rf22))